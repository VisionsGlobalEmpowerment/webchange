...
(deftest activity-stat-created-on-finish-activity
  (let [{:keys [course-slug course-id user-id student-id]} (fp/course-stat-created)
        activity (activity-finished)
        data (progress-with-event activity)
        _ (fp/save-current-progress! user-id course-slug data)
        retrieved (-> (fp/get-individual-profile student-id course-id) :body slurp (json/read-str :key-fn keyword) :stats (stats-for activity))]
...
(deftest time-spent-increased-on-activity-stopped
  (let [{:keys [user-id student-id course-slug course-id]} (fp/activity-stat-created (select-keys (activity-stopped) [:activity :lesson :level]))
        _ (fp/save-current-progress! user-id course-slug (progress-with-event (activity-stopped)))
        retrieved (-> (fp/get-individual-profile student-id course-id) :body slurp (json/read-str :key-fn keyword) :stats (stats-for (activity-stopped)))]
    (is (= (:time-spent (activity-stopped)) (-> retrieved :data :time-spent)))))

(deftest time-spent-increased-on-activity-finished
  (let [{:keys [user-id student-id course-slug course-id]} (fp/activity-stat-created (select-keys (activity-finished) [:activity :lesson :level]))
        _ (fp/save-current-progress! user-id course-slug (progress-with-event (activity-finished)))
        retrieved (-> (fp/get-individual-profile student-id course-id) :body slurp (json/read-str :key-fn keyword) :stats (stats-for (activity-finished)))]

...
(deftest test-prepare-phrase-actions--invalid-action
  (let [params {:action-path  [:dialog-main]
                :scene-data   scene-data
                :concept-data concept-data}
        actual-result (get-phrases-sequence params)
        expected-result [{:scene-action-path [:dialog-main :data 0]
                          :parallel-mark     :none}
                         {:scene-action-path [:dialog-main :data 1]
                          :parallel-mark     :none}
                         {:scene-action-path [:dialog-main :data 2]
                          :parallel-mark     :none}]]
    (compare-results! actual-result expected-result)))

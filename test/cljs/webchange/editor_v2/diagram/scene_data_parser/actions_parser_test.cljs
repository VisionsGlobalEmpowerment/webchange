(ns webchange.editor-v2.diagram.scene-data-parser.actions-parser-test
  (:require
    ...
    [webchange.editor-v2.diagram.scene-data-parser.actions-parser :refer [merge-actions
                                                                          parse-action
                                                                          parse-actions-chain
                                                                          seq-intersection]]))

(defn get-keys-compare
  [actual expected]
  (reduce
    (fn [res key]
    ...
    (let [action-name :click-on-box1
      ...
      (is (= (parse-action action-name action-data parent-action next-action prev-action)
             {:click-on-box1 {:type        "test-var-scalar"
                              :connections {:previous-action {:next [:first-word
                                                                     :pick-wrong]}}
                              :data        action-data}})))))
    ...
    (let [action-name :first-word
      ...
      (is (= (parse-action action-name action-data parent-action next-action prev-action)
             {:first-word {:type        "sequence"
                           :connections {:previous-action {:next [:show-first-box-word]}}
                           :data        action-data}}))))
      ...
      (is (= (parse-actions-chain actions-data start-node-name start-node-data parent-action next-action prev-action)
             {:simple-parallel   {:type        "parallel"
                                  :data        {:type "parallel"
                                                :data [{:type "action"}
                                                       {:type "action"}]}
                                  :connections {:previous-action {:next [:simple-parallel-0
                                                                         :simple-parallel-1]}}}
             ...
             {:simple-seq   {:type        "sequence"
                             :data        {:type "sequence"
                                           :data ["sub-action-0"
                                                  "sub-action-1"
                                                  "sub-action-2"]}
                             :connections {:previous-action {:next [:sub-action-0]}}}
              :sub-action-0 {:type        "action"
                             :data        {:type "action"}
    ...
    (let [actions-data {:action-seq-1   {:type "sequence"
      ...
      (let [actual-result (parse-actions-chain actions-data start-node-name start-node-data parent-action next-action prev-action)
            expected-result {:action-seq-1   {:type        "sequence"
                                              :connections {:previous-action {:next [:action-seq-1-1]}}
                                              :data        {:type "sequence"
                                                            :data ["action-seq-1-1"
                                                                   "action-seq-2"
                                                                   "action-seq-1-2"]}}
                             :action-seq-2   {:type        "sequence"
                                              :connections {:action-seq-1-1 {:next   [:action-seq-2-1]
                                                                             :parent :action-seq-1}}
                                              :data        {:type "sequence"
                                                            :data ["action-seq-2-1"
                                                                   "action-seq-2-2"]}}
                             :action-seq-1-1 {:type        "empty"
                                              :connections {:action-seq-1 {:next   [:action-seq-2]
                                                                           :parent :action-seq-1}}
                                              :data        {:type "empty"}}
                             :action-seq-1-2 {:type        "empty"
                                              :connections {:action-seq-2-2 {:next   [:next-action]
                                                                             :parent :action-seq-1}}
                                              :data        {:type "empty"}}
                             :action-seq-2-1 {:type        "empty"
                                              :connections {:action-seq-2 {:next   [:action-seq-2-2]
                                                                           :parent :action-seq-2}}
                                              :data        {:type "empty"}}
                             :action-seq-2-2 {:type        "empty"
                                              :connections {:action-seq-2-1 {:next   [:action-seq-1-2]
                                                                             :parent :action-seq-2}}
                                              :data        {:type "empty"}}}]

        (when-not (= actual-result expected-result)
          (println (get-keys-compare actual-result expected-result)))
      ...
      (is (= (parse-actions-chain actions-data start-node-name start-node-data parent-action next-action prev-action)
             {:simple-test    {:type        "test-var-scalar"
                               :data        {:type     "test-var-scalar"
                                             ...
                                             :success  "success-action"
                                             :fail     "fail-action"}
                               :connections {:previous-action {:next [:success-action :fail-action]}}}
              :success-action {:type        "action"
                               :data        {:type "action"}
                               :connections {:simple-test {:next []}}}
              :fail-action    {:type        "action"
                               :data        {:type "action"}
                               :connections {:simple-test {:next []}}}}))
      )))
...
(deftest test-merge-actions-map
  ...
  (testing "simple-merge"
    (let [map-1 {:action-1 {:type        "action"
                            :connections {:connect-1 {:next [:next-1]}}}}
          map-2 {:action-2 {:type        "action"
                            :connections {:connect-2 {:next [:next-2]}}}}]

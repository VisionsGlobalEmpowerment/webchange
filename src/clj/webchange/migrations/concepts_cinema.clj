(ns webchange.migrations.concepts-cinema
  (:require
...
(defn- set-created-params
  [data src]
  (-> data
      (assoc-in [:actions :init-concept :var-value] src)
      (update :assets concat [{:url src :type "video"}])
      (assoc-in [:metadata :history :created :video-src] src)))
...
(defn- process-scene
  ...
  (let [course-data (-> (db/get-latest-course-version {:course_id course-id})
        ...
        activity-data (-> scene-data
                          ...
                          (set-created-params (:chanting-video-src concept)))

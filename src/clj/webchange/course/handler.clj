...
(defroutes course-routes
  ...
  (POST "/api/courses/:course-id/duplicate" request
    ...
    (let [user-id (current-user request)]
      (when-not (is-super-admin? user-id)
        (throw-unauthorized {:role :educator}))
  ...
  (GET "/api/classes/:class-id/course" request
    ...
    (let [user-id (current-user request)
          {school-id :id} (school/get-school-by-class class-id)]
      (when-not (or (is-admin? user-id)
                    (school/school-admin? school-id user-id)
                    (school/class-teacher? class-id user-id))
        (throw-unauthorized {:role :educator}))

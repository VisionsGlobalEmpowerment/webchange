...
(events/reg
 ...
 (fn [{:keys [user-id course-id unique-id score time-spent created-at course-in-progress] :or {time-spent 0}}]
...
(events/reg
 ...
 (fn [{:keys [user-id course-id unique-id time-spent created-at course-in-progress] :or {time-spent 0}}]
   (when course-in-progress
     (if-let [{:keys [id data]} (db/get-activity-stat {:user_id user-id :course_id course-id :unique_id unique-id})]
       (do
         (db/save-activity-stat! {:id id :data (-> data

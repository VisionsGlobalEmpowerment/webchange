...
(events/reg
  ...
  (fn [{:keys [user-id course-id level lesson activity-name]}]
    (if-let [{:keys [id data]} (db/get-user-course-stat {:user_id user-id :course_id course-id})]
      (db/save-course-stat! {:id id :data (assoc data :latest-activity {:id activity-name :level level :lesson lesson})}))))

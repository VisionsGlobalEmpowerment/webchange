...
(def map-scene {:assets        [{:url "/raw/img/map/background.jpg", :size 10, :type "image"}
                ...
                :objects       {:background {:type "background", :src "/raw/img/map/background.jpg"},
                                ...
                                :cinema     {:type    "image" :width 230 :height 210 :x 1585 :y 469
                                             :scale   {:x 1 :y 1}
                                             :src     "/raw/img/map/Cine_Enabled.png"
                                             :actions {:click {:type "action", :id "move-to" :params {:scene-id "cinema"}, :on "click"}}}},

                :scene-objects [["background" "home" "feria" "park" "library" "stadium" "cinema"] ["vera"]],
                :actions       {:move-to                {:type        "test-value"
                                ...
                                :move-from-cinema       {:type "sequence-data"
                                                         :data [{:type "start-animation" :target "vera-go"}
                                                                {:type "transition" :transition-id "vera-transition" :to {:x 1384, :y 730}}
                                                                {:type "transition" :transition-id "vera-transition" :to {:x 1210, :y 655}}
                                                                {:type "transition" :transition-id "vera-transition" :to {:x 1070 :y 665}}]}

                                :move-to-cinema         {:type "sequence-data"
                                                         :data [{:type "start-animation" :target "vera-go"}
                                                                {:type "transition" :transition-id "vera-transition" :to {:x 1210, :y 655}}
                                                                {:type "transition" :transition-id "vera-transition" :to {:x 1384, :y 730}}
                                                                {:type "transition" :transition-id "vera-transition" :to {:x 1627 :y 672}}]}

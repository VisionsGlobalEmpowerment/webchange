...
(defn- classes-list-menu-item
  ...
  [ui/list-item {:on-click       #(on-click id)
                 :style          {:text-transform "capitalize"}}
   [ui/list-item-text {:inset true} name]])
              ...
              (let [current-class (r/atom {})
                ...
                (fn []
                  (let [classes @(re-frame/subscribe [::classes-subs/classes-list])
                    ...
                    [ui/list
                     [ui/list-item {:on-click     #(re-frame/dispatch [::classes-events/show-manage-classes])}
                      [ui/list-item-text {} "Classes"]]
                     (for [class classes]
                       ^{:key (:id class)}
                       [classes-list-menu-item class {:on-click show-class}])
                     [ui/list-item {:on-click     (fn [] (do (reset! current-class {})

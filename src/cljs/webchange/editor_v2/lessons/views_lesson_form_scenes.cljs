...
(defn- get-styles
  ...
  {:action-icon             {:font-size "1.4rem"}
   ...
   :activity-name           {:flex       "1 1 auto"
                             :text-align "left"}
   :activity-time           {:width      "130px"
                             :padding    "0 16px"
                             :flex       "0 0 auto"
                             :text-align "center"}
   :activity-scored         {:width      "90px"
                             :padding    "0 16px"
                             :flex       "0 0 auto"
                             :text-align "center"}
   :activity-expected-score {:width      "150px"
                             :padding    "0 16px"
                             :flex       "0 0 auto"
                             :text-align "center"}
...
(defn- list-header
  ...
  (let [items {:activity-name           "Activity"
               :activity-time           "Time Expected"
               :activity-scored         "Scored"
               :activity-expected-score "Expected Score %"}
        styles (get-styles)]
  ...
  (let [styles (get-styles)]
    [ui/list-item {:button   true
     ...
     [ui/typography {:variant "body1"
      ...
      (-> scene :scored boolean str)]
     [ui/typography {:variant "body1"
                     :style   (:activity-expected-score styles)}
      (:expected-score-percentage scene)]
     [:div {:style (:activity-actions styles)}
...
(defn lesson-scenes
  ...
  (r/with-let [current-scene-index (r/atom nil)]
              (let [items (keep-indexed vector (:activities @data))
                    close-modal #(reset! current-scene-index nil)
